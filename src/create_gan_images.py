"""
Create GAN images from white Gaussian noise.
"""
import argparse
import os.path
...
def parseargs():
    ...
    parser.add_argument('-g',
                        ...
                        default="../gan/generator.pt")
    parser.add_argument('-o',
                        '--out_dir',
                        type=str,
                        help="Output directory",
                        default="../demo_out/gan")
...
class Interpolate(torch.nn.Module):
    ...
    def __str__(self):
        return ("Interpolate(size=" + str(self.size) + "," + str(self.mode) + "," +
                "align_corners=" + str(self.align_corners) + ")")
    def __repr__(self):
        return self.__str__()
...
def _gen_conv_block(in_channels, out_channels, kernel_size, stride, padding):
    """
    Convolution building block for generator.
    """
...
def _gen_interp_block(interp_size, features_in, mode='bilinear', align_corners=True):
    """
    Interpolation building block for generator.
    """
    block = torch.nn.Sequential(
...
if __name__ == "__main__":
    ...
    model.eval()
    # Create images
    for cl_int_label,cls in enumerate(("control", "coronal", "metopic", "sagittal")):
