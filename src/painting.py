...
def rescale_image_array(dist_map_list,scaling_method="linear"):
    """
...
class ImageFromDistancesCreator():
    ...
    def __call__(self,dist_map_list=None):
        ...
        dist_map_list = rescale_image_array(dist_map_list,scaling_method=self.scaling_method)
        if self.correct_image_outliers:
            # dist_map_list_orig = dist_map_list
            # print("Before", dist_map_list)
            dist_map_list = [set_image_values_outside_boundary(dist_map,target_min=self._target_min,target_max=self._target_max) for dist_map in dist_map_list]
...
def none_helper(y):
    ...
    From: https://stackoverflow.com/questions/39452792/cannot-cast-array-data-from-dtypeo-to-dtypefloat64
    Input:
    ...
    """
    y = numpy.array(y)
    return numpy.array([x == None for x in y]), lambda z: numpy.array(z).nonzero()[0]

def inpaint_nans_1d(y):
    nans,fx = none_helper(y)
    y = numpy.array(y,dtype='float64')
    y[nans]= numpy.interp(fx(nans), fx(~nans), y[~nans])
    return y
